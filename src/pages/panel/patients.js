...
const Patients = ({ dbPatients }) => {
  ...
  useEffect(() => {
    ...
    let filteredPatients = dbPatients.filter((item)=>{
  ...
  const submit = async(e)=>{
    ...
    const data = { patientData, path:'Patients' }
    ...
    if (response.success === true) {
      ...
      setOpen(false)
      setFilteredPatients([...filteredPatients, response.data]);
    }
    else {
  ...
  const editEntry = async()=>{
    const data = { id, patientData, path: 'Patients'};
  ...
  const delEntry = async()=>{
    const data = { selectedIds , path: 'Patients' };
    ...
    if (response.success === true) {
      setFilteredPatients(filteredPatients.filter(item => !selectedIds.includes(item._id)));
    }
    else {
      toast.error(response.message , { position: "top-right", autoClose: 1000, hideProgressBar: false, closeOnClick: true, pauseOnHover: true, draggable: true, progress: undefined, theme: "light", });
    }
            ...
            <CardBody>
              ...
              <div className='flex justify-between items-center py-2'>
                <Typography variant="h5" color="blue-gray" className="flex items-center">
                  <FaMaskFace className='mr-2 text-xl' /> Patients ({filteredPatients.length})
                </Typography>
                  ...
                  <div className="-m-1.5 overflow-x-auto">
                    <div className="inline-block align-middle w-full">
                      <div className="block h-[21rem]">
                            ...
                            {filteredPatients.length != 0 && filteredPatients.map((item, index)=>{
                              ...
                              onClick={(e) => {
                                if (!e.target.tagName.toLowerCase() === 'input' || e.target.type !== 'checkbox') {
                                  e.stopPropagation(); setPatientData(item), setOpen(true), setId(item._id), setIsEdit(true)
                                }
                              }}
                              ...
                              <td className="text-start px-3 py-2 whitespace-nowrap text-xs font-medium text-gray-800">{item.userID}</td>
                              ...
                              <td className="text-start px-3 py-2 whitespace-nowrap text-xs text-gray-800">
                                {moment(item.dob).format('D MMM YYYY')}
                              </td>
                              <td className="text-start px-3 py-2 whitespace-nowrap text-xs text-gray-800">{item.address}</td>
                        ...
                        {filteredPatients.length === 0 && <div className='w-full'>
...
export async function getServerSideProps() {
  ...
  let dbPatients = await User.find();

  // Pass data to the page via props
  return {
    props: {
      dbPatients: JSON.parse(JSON.stringify(dbPatients)),
    }
  }
}
